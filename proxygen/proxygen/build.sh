...
function setup_libtins() {
  TINS_DIR=$DEPS_DIR/libtins
  TINS_BUILD_DIR=$DEPS_DIR/libtins/build/
  if [ ! -d "$TINS_DIR" ] ; then
    echo -e "${COLOR_GREEN}[ INFO ] Cloning libtins repo ${COLOR_OFF}"
    git clone https://github.com/mfontanini/libtins.git "$TINS_DIR"
  fi
  echo -e "${COLOR_GREEN}Building libtins ${COLOR_OFF}"
  mkdir -p "$TINS_BUILD_DIR"
  cd "$TINS_BUILD_DIR" || exit
  ...
  if [ "$STATIC" == true ] ; then
    MAYBE_BUILD_SHARED_LIBS="-DLIBTINS_BUILD_SHARED=0"
  fi
...
function setup_libtuntap() {
  TUNTAP_DIR=$DEPS_DIR/libtuntap
  TUNTAP_BUILD_DIR=$DEPS_DIR/libtuntap/build/
  if [ ! -d "$TUNTAP_DIR" ] ; then
    echo -e "${COLOR_GREEN}[ INFO ] Cloning libtuntap repo ${COLOR_OFF}"
    git clone https://github.com/c-rotte/libtuntap.git "$TUNTAP_DIR"
    cd "$TUNTAP_DIR" && git checkout bugfix/tuntap_sys_set_ifname
  fi
  echo -e "${COLOR_GREEN}Building libtuntap ${COLOR_OFF}"
  mkdir -p "$TUNTAP_BUILD_DIR"
  cd "$TUNTAP_BUILD_DIR" || exit
  ...
  make -j "$JOBS"
  make install
  echo -e "${COLOR_GREEN}libtuntap is installed ${COLOR_OFF}"
  cd "$BWD" || exit
}

function setup_easy_profiler() {
  PROFILER_DIR=$DEPS_DIR/easy_profiler
  PROFILER_BUILD_DIR=$DEPS_DIR/easy_profiler/build/
  if [ ! -d "$PROFILER_DIR" ] ; then
    echo -e "${COLOR_GREEN}[ INFO ] Cloning easy_profiler repo ${COLOR_OFF}"
    git clone https://github.com/yse/easy_profiler.git "$PROFILER_DIR"
  fi
  echo -e "${COLOR_GREEN}Building easy_profiler ${COLOR_OFF}"
  mkdir -p "$PROFILER_BUILD_DIR"
  cd "$PROFILER_BUILD_DIR" || exit
  ...
  make -j "$JOBS"
  make install
  echo -e "${COLOR_GREEN}easy_profiler is installed ${COLOR_OFF}"
  cd "$BWD" || exit
}
...
function setup_gperftools() {
  GPERFTOOLS_DIR=$DEPS_DIR/gperftools
  GPERFTOOLS_BUILD_DIR=$DEPS_DIR/gperftools/build/
  if [ ! -d "$GPERFTOOLS_DIR" ] ; then
    echo -e "${COLOR_GREEN}[ INFO ] Cloning gperftools repo ${COLOR_OFF}"
    git clone https://github.com/gperftools/gperftools.git "$GPERFTOOLS_DIR"
  fi
  echo -e "${COLOR_GREEN}Building gperftools ${COLOR_OFF}"
  mkdir -p "$GPERFTOOLS_BUILD_DIR"
  cd "$GPERFTOOLS_BUILD_DIR" || exit
  cmake -DCMAKE_BUILD_TYPE=RelWithDebInfo       \
    -DCMAKE_PREFIX_PATH="$DEPS_DIR"             \
    -DCMAKE_INSTALL_PREFIX="$DEPS_DIR"          \
    -DBUILD_SHARED_LIBS=OFF                     \
  ...
  make -j "$JOBS"
  make install
  echo -e "${COLOR_GREEN}gperftools is installed ${COLOR_OFF}"
  cd "$BWD" || exit
}

function setup_mvfst() {
  MVFST_BUILD_DIR=$MVFST_DIR/_build/
  if [ ! -d "$MVFST_DIR" ] ; then
    echo -e "${COLOR_RED}[ ERROR ] Couldn't find the mvfst repo ${COLOR_OFF}"
    exit -1
  fi
...
PREFIX=""
COMPILER_FLAGS=""
USAGE="./build.sh [-j num_jobs] [-q|--with-quic] [-m|--no-jemalloc] [--no-install-dependencies] [-p|--prefix] [-x|--compiler-flags] [--no-fetch-dependencies] [--masque] [--ci-static]"
while [ "$1" != "" ]; do
  case $1 in
    -j | --jobs ) shift
                  JOBS=$1
...
cmake                                           \
  -DCMAKE_BUILD_TYPE=RelWithDebInfo             \
  -DCMAKE_PREFIX_PATH="$DEPS_DIR;$MVFST_DIR"    \
...
if [ "$MASQUE" == true ] ; then
  ...
  echo -e "${COLOR_GREEN}Building proxygen_masque_server..."
  make -j "$JOBS" proxygen_masque_server
  echo -e "${COLOR_GREEN}Building proxygen_masque_client..."
  make -j "$JOBS" proxygen_h3datagram_client
  echo -e "${COLOR_GREEN}Building proxygen_masque_http_client..."
  make -j "$JOBS" proxygen_masque_http_client
  echo -e "${COLOR_GREEN}Building MasqueTests..."
  make -j "$JOBS" MasqueTests
  echo -e "${COLOR_GREEN}Building hq..."
  make -j "$JOBS" hq
