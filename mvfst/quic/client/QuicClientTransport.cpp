...
void QuicClientTransport::setSupportedExtensionTransportParameters() {
  const auto containsParameter = [this](TransportParameterId parameterId) {
    auto it = std::find_if(
        customTransportParameters_.begin(),
        customTransportParameters_.end(),
        [&parameterId](const auto& parameter) {
          return parameter.parameter == parameterId;
        });
    return it != customTransportParameters_.end();
  };
  if (conn_->transportSettings.minAckDelay.hasValue() &&
      !containsParameter(TransportParameterId::min_ack_delay)) {
  ...
  if (conn_->transportSettings.datagramConfig.enabled &&
      !containsParameter(TransportParameterId::max_datagram_frame_size)) {
  ...
  if (conn_->transportSettings.advertisedKnobFrameSupport &&
      !containsParameter(TransportParameterId::knob_frames_supported)) {
