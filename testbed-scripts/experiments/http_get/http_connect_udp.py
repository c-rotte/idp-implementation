...
def http_connect_udp_factory(num_hops, num_transactions):
    assert 0 <= num_hops <= 4, "Number of hops must be between 0 and 4"

    CLASS_NAME = f"HttpGETConnectUDP{num_hops:03d}x{num_transactions:03d}"
    ...
    class HttpClient(Experiment):
        ...
        def _pre(self):
            ...
            self.http_server_pos_id = utils.execute_command_on_node(Command(
                command=f"./host_scripts/http_server.sh "
                        "1337 "
                        f"{self.cc.lower()} "
                        f"{1472 - (len(self.proxy_nodes) * 58)} "
                        "0 "  # threads = nCPU
        ...
        def _start(self):
            ...
            paths = []
            for i in range(len(self.proxy_nodes)):
                target_host = (hosts + [self.server_node.address_eno3])[1:][i]
                target_port = (ports + [1337])[1:][i]
                paths.append(f"/.well-known/masque/udp/{target_host}/{target_port}")
            UDPSendPacketLens = [1472 - (i * 58) for i in range(len(self.proxy_nodes))]
            maxRecvPacketSizes = [1472 - (i * 58) for i in range(len(self.proxy_nodes))]
            ...
            ./binaries_proxygen/masque-http-client
              ...
              --method GET
              --timeout 9999999999
              --UDPSendPacketLen {1472 - (len(self.proxy_nodes) * 58)}
              --maxRecvPacketSize {1472 - (len(self.proxy_nodes) * 58)}
            ...
            if len(hosts) > 0:
                command += f"""
                  --cc None
                  --modes {' '.join(["connect-udp"] * len(hosts))} 
                  --hosts {' '.join(hosts)} 
...
for num_hops in range(0, MAX_HOPS + 1):
    for num_transactions in utils.exponential_range(1, MAX_TRANSACTIONS + 1):
        class_name = f"HttpConnectUDP{num_hops}x{num_transactions}"
        globals()[class_name] = http_connect_udp_factory(num_hops, num_transactions)
